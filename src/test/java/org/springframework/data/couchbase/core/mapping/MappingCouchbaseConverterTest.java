...
public class MappingCouchbaseConverterTest {
  ...
  public void writesAndReadsValueClass() {
    ...
    ValueEntity entity = new ValueEntity("foo@bar.com");
    converter.write(entity, converted);

    Map<String, Object> expected = new HashMap<String, Object>();
    expected.put("_class", entity.getClass().getName());
    expected.put("email", new HashMap<String, Object>() {{
      put("emailAddr", "foo@bar.com");
    }});
    ...
    CouchbaseDocument email = new CouchbaseDocument();
    email.put("emailAddr", "foo@bar.com");
    source.put("email", email);
    CouchbaseList listOfEmails = new CouchbaseList();
    listOfEmails.put(email);
    source.put("listOfEmails", listOfEmails);

    ValueEntity readConverted = converter.read(ValueEntity.class, source);
    System.out.println(readConverted.email);
    System.out.println(readConverted.listOfEmails);
  }
  ...
  static class ValueEntity extends BaseEntity {
    ...
    public ValueEntity(String email, List<Email> listOfEmails) {
      this.email = new Email(email);
      listOfEmails.add(this.email);
    }

    public ValueEntity(String email) {
      this(email, new ArrayList<Email>());
    }

    public void setListOfEmails(List<Email> listOfEmails) {
